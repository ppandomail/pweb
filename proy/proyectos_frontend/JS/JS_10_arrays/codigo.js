...
letters.every((e) => e.length === 1); // true, verifica si c/u cumple con la condición

letters = ["a", "bb", "c", "d"];
letters.some((e) => e.length == 2); // true, verifica si alguno cumple con la condición

names = ["Ana", "Pablo", "Pedro", "Pancracio", "Heriberto"];
names.map((e) => e.length); // Devuelve [3, 5, 5, 9, 9], transforma los elementos
...
numeros_pares = [2, 6, 4];
numeros_pares.concat([10, 8]); // [2, 6, 4, 10, 8]
numeros_pares.sort(); // [2, 4, 6]
numeros_pares.reverse(); // [4, 6, 2]
